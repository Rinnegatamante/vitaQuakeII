...
extern cvar_t *scale_2d;

float SCR_GetMenuScale(void)
{
	if (scale_2d && !scale_2d->value) return 1.0f;
	else if (scr_width > 720) return 1.5f;
	else if (scr_width > 640) return 1.125f;
	else return 1.0f;
}
...
void Menu_DrawCenteredString( int y, const char *string )
{
	unsigned i;
	float scale = SCR_GetMenuScale();
	unsigned x = VID_WIDTH / 2 - 4 * strlen(string) * scale;
	for ( i = 0; i < strlen( string ); i++ )
	{
		Draw_Char( ( x + i*8*scale ), scale*y, string[i], scale );
	}
...
void Slider_Draw( menuslider_s *s )
{
	...
	Draw_Char(s->generic.x + (s->generic.parent->x + RCOLUMN_OFFSET * scale),
			(s->generic.y + s->generic.parent->y) * scale, 128, scale);

	for (i = 0; i < SLIDER_RANGE * scale; i++)
	{
		Draw_Char((RCOLUMN_OFFSET * scale + s->generic.x + i * 8 + s->generic.parent->x + 8),
			   	(s->generic.y + s->generic.parent->y) * scale, 129, scale);
	}

	Draw_Char((RCOLUMN_OFFSET * scale + s->generic.x + i * 8 +
